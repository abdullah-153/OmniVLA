...
Explicit absolute or relative file locations are looked up exactly. A missing location does not fall back to a similarly named file elsewhere. Quote paths containing spaces. Web URLs ending in a supported text extension are handled by the web reader, not by local discovery.
